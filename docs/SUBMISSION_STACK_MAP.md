...
| Conversation output hygiene | Model drafting, role-marker, or mid-thought artifacts can appear as if they are a real answer. | System-level guidance and a bounded response screen keep obvious scaffolding out of stored replies and use the existing fallback ladder when needed. | |
