...
3. **Write an entry.** The browser shows a local preview of possible sensitive content, but the server performs the trusted scan. Entries are limited to 8,000 characters and use a client request ID to prevent duplicate saves.
...
6. **Continue the entry conversation.** Replies are limited to 2,000 characters, stored under that entry, and sent through the same Privacy Guardian and fallback pipeline. The full thread remains available to the owner, while only the latest ten turns are sent to Gemini for bounded cost and latency.
