...
- [ ] Multi-stage Dockerfile builds the Vite frontend and TypeScript server.
- [ ] Runtime image contains only production dependencies, compiled server code, and compiled frontend files.
- [ ] Docker build requires the six Firebase browser configuration values.
- [ ] Docker build supports production App Check or an explicitly disabled staging pass.
- [ ] Runtime secrets are not Docker build arguments.
- [ ] .dockerignore excludes environment files, generated output, logs, and service-account key files.
- [ ] .gcloudignore excludes the same sensitive and generated files from the Cloud Build source archive.
- [ ] The server production image copies only server/src and does not copy server/key.json.
- [ ] cloudbuild.yaml passes only public Firebase/App Check values to Docker build arguments.
- [ ] Docker uses neutral build-argument names and shell-scoped Vite variables, so BuildKit does not misclassify public browser configuration as runtime secrets.
- [ ] Vite splits Firebase vendor code from the application chunk; the production build is below the 500 kB warning threshold.
- [ ] Provisioning script uses a separate build service account and Cloud Run runtime service account.
...
- [ ] Final warning-free Docker image smoke passes: `/healthz`, `/health`, `/`, and the generated JavaScript asset return HTTP 200; startup logs contain only the expected listener message.
- [ ] Production App Check build gate passes when a non-empty site key is supplied.
- [ ] Production-shaped runtime smoke passes with `ENFORCE_APP_CHECK=true`: `/healthz` and the frontend both return HTTP 200.

### Google Cloud prerequisites

- [ ] gcloud installed and authenticated through the verified portable Google Cloud CLI archive.
...
- [ ] Firestore database exists in asia-southeast1 and is the native `(default)` database.
- [ ] Production web Firebase configuration collected from the target Firebase app.
- [ ] Firebase App Check Web-app registration and allowed-domain configuration independently confirmed in the Firebase Console.
- [ ] GEMINI_API_KEY exists in Secret Manager and is bound at runtime.
- [ ] DELETION_HMAC_KEY exists in Secret Manager and is bound at runtime.
- [ ] RETENTION_WORKER_TOKEN exists in Secret Manager and is bound at runtime.
- [ ] Deployer can create service accounts, bind IAM roles, submit builds, and deploy Cloud Run.

### Live verification

- [ ] Cloud Build image build completes and image digest is recorded.
- [ ] Cloud Run revision uses personal-gemini-journal-run.
- [ ] Cloud Run runtime secret bindings are present without exposing values.
- [ ] `/health` returns ok on the current staging revision; the exact `/healthz` path is intercepted by the Cloud Run front end.
...
- [ ] Firestore rules and the retention collection-group index are deployed and ready.
- [ ] Scheduler exists and is configured for /internal/retention/redact.
- [ ] A real due retention record is eventually observed as entry_redacted.
- [ ] Required Cloud Run label is present.
- [ ] Live URL, image digest, revision, and staging verification results are recorded below.
