...
- Preserve the current entry contract: 8,000-character entry limit, required client request ID, server-side Privacy Guardian, structured Gemini summary/topics/categories/reflection, RAW/DERIVED separation, hash-chain fields, and graceful save when Gemini fails.
...
- Preserve the current reply contract: 2,000-character reply limit, required client request ID, Privacy Guardian on every AI-mode reply, latest-10-turn Gemini context, user/model conversation records, `409` rejection for Private Journal entries, and `410` rejection for deleted or deleting entries.
