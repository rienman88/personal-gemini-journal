...
Current recorded result as of 2026-09-06: the build passes, the browser smoke suite has 6 passing tests, and the emulator-backed server suite has 45 passing and 2 intentionally pending tests. The pending tests are the live Gemini authenticity check without `GEMINI_API_KEY_TEST` and the named route-level idempotency specification awaiting a complete route harness. AI mode policy tests additionally verify the server-enforced Private Journal no-Gemini branch; browser coverage also verifies private-note controls and accordion behavior. Conversation output hygiene is covered by a regression test that rejects obvious drafting artifacts and uses the existing fallback ladder.
