...
| 64 | Conversation Output Hygiene | In AI Journal, submit a reply that produces a controlled drafting artifact in the test double, then allow the fallback model to answer. In a live walkthrough, submit a normal reply and inspect the rendered conversation. | Conversation guidance is applied as a system instruction; obvious drafting, reasoning, role-marker, and slash-command artifacts are rejected before persistence; the fallback ladder produces a direct bounded reply. | [x] PASSED - automated regression; [ ] Live reply walkthrough READY TO RUN |
...
| Firebase Auth and Firestore emulator suite | 45 passed, 2 intentionally pending on 2026-09-06 |
...
| Gemini resilience | Passed for schema retry, model fallback, non-retryable failure, empty response, drafting-artifact rejection, and bounded attempts |
